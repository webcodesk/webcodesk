...
Для этого просто сделайте два клика по названию компонента в разделе `Components`. Появится новая закладка, в которой будет отображен компонент с установленными свойствами по-умолчанию.

***{IMAGE}***

В просмотре компонента есть возможность прочитать описание компонента на закладке `About`, которое было собрано на основе комментариев в исходном коде.

Если у вас есть необходимость проверить компонент с разными настройками, у вас есть возможность создавать истории для компонента. Это работает по аналогии с React StoryBook.

Как пример, кликните два раза на компоненте `Form`. В открывшемся окне перейдите на закладку `Stories`, вы увидите два пункта в списке историй. Первый - это компонент с дефолтными свойствами. И второй - это кастомные настройки свойств компонента.
...
Одной из очень полезных возможностей в Webcodesk является возможность записать и просмотреть какие данные были переданы по цепочкам в диаграммах.
